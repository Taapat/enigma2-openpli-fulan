...
class ClockToText(Converter, object):
	...
	def getText(self):
		...
		# handle durations
		if self.type is self.IN_MINUTES:
			return _("%d min") % (time / 60)
		elif self.type is self.AS_LENGTH:
			if time < 0:
				return ""
			return "%d:%02d" % (time / 60, time % 60)
		elif self.type is self.AS_LENGTHHOURS:
			if time < 0:
				return ""
			return "%d:%02d" % (time / 3600, time / 60 % 60)
		elif self.type is self.AS_LENGTHSECONDS:
			if time < 0:
				return ""
			return "%d:%02d:%02d" % (time / 3600, time / 60 % 60, time % 60)
		elif self.type is self.TIMESTAMP:
			return str(time)

		t = localtime(time)

		if self.type is self.WITH_SECONDS:
		...
		elif self.type is self.SHORT_DATE:
			# TRANSLATORS: short date representation short dayname daynum short monthname in strftime() format! See 'man strftime'
			d = dayOfWeek[t[6]] + _(" %d/%d") % (t[2], t[1])
		elif self.type is self.LONG_DATE:
